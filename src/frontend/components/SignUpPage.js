    ...
    function AcpSignUp() {
        ...
        for (let i = 0; i < document.getElementsByClassName("signup").length; i++) {
            if (document.getElementsByClassName("signup")[i].value === "") {
                ...
                setTimeout(()=>{document.getElementsByClassName("error")[0].style.display = 'none';},1000)
                check += 1;
            }
        }
        if (check === 0) {
    ...
    return (
        <div className="signup-page" id="signup-page">
            <div className="background">
                ...
                <Link to='/login'><p className="signup-btn" id="signup-acp" onClick={AcpSignUp} >Sign up</p></Link>
            </div>
        </div >
    )
}
